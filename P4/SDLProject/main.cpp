...
void Initialize() {
	...
	for (int i = 11; i < 17; i++) {
		state.platforms[i].entityType = PLATFORM;
		state.platforms[i].textureID = platformTextureID;
		state.platforms[i].position = glm::vec3(16 - i, -0.75f, 0);
	}

	for (int i = 17; i < PLATFORM_COUNT; i++) {
		state.platforms[i].entityType = PLATFORM;
		state.platforms[i].textureID = platformTextureID;
		state.platforms[i].position = glm::vec3(i - 23, 1.75f, 0);
	}
	...
	state.enemies[0].position = glm::vec3(4, 0.25, 0);
	...
	state.enemies[2].position = glm::vec3(-5, 2.75, 0);
...
void Render() {
	...
	if (state.player->isActive == false) {
		DrawText(&program, fontTextureID, "You Lose!", 1.0f, -0.6f, glm::vec3(-1.6f, 0, 0));
	}
	else if (state.player->killCount == ENEMY_COUNT) {
		DrawText(&program, fontTextureID, "You Win!", 1.0f, -0.6f, glm::vec3(-1.0f, 0, 0));
	}
